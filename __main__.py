...
debugOpts.add_argument("--checkpoints", help="enable checkpoints", action="store_true")
    ...
    try:
        simulation = errors.callBeginningScenicTrace(
            lambda: simulator.simulate(
                ...
                enableCheckpoint=args.checkpoints
            )
        )
        replay = simulation.getReplay()
        print('replay obtained')
        print('timestep : ',simulation.result.trajectory[20])
        if args.checkpoints:
            simulation2 = errors.callBeginningScenicTrace(
                lambda: simulator.simulate(
                    scene,
                    # replay,
                    maxSteps=args.time,
