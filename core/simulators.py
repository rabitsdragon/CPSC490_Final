    ...
    def simulate(
        ...
        *,
        ...
        Args:
            ...
            enableCheckpoint (bool): Whether a save-state will be kept within the simulation.
                On simulation end, the user will have the choice to return to save state, or
                end the simulation.
...
class Simulation(abc.ABC):
        ...
        *,
        ...
        try:
            ...
            if checkpoint:
                ...
                print(checkpoint['trajectories'][0])
                # print(self.checkpoint['trajectories'])
                for i in range(len(self.objects)):
                    self.objects[i].position = checkpoint['trajectories'][i]
    ...
    def _run(self, dynamicScenario, maxSteps):
        ...
        while True:
            ...
            if self.enableCheckpoint:
                self.createCheckpoint(self.currentTime)#, replay)
    
    def createCheckpoint(self, time):
        """Saves the current point of the current simulation.
        """
        if time % (4) != 0: # We shouldn't create a new checkpoint yet.
            return
        self.checkpoint = {}
        self.checkpoint['time'] = time
        self.checkpoint['objects'] = self.scene.objects
        self.checkpoint['trajectories'] = self.trajectory[-1]

    def resetReplay(self, replay, allowPickle=True):
        """
        self.recordCurrentState() # Records CurrentState and keeps results in trajectory
        # This is already being saved in the end state
        
        self.currentState() # Returns currentState of a list of all the objects as a tuple

        # Can I call simulate within the Simulation class?
        """
...
class SimulationResult:
    ...
    def __init__(self, trajectory, actions, terminationType, terminationReason, records, checkpoint=None):
